...
class CodeAgentRun(BaseModel):
    """Represents the complete state of a code agent execution, allowing it to be saved and resumed."""
    run_id: str = Field(description="A unique identifier for this run, typically a timestamp or UUID.")
    task: str = Field(description="The final user-provided task description.")
    original_branch: str = Field(description="The git branch the user was on when the run was started.")
    analysis: CodeAnalysis = Field(description="The complete analysis and plan generated by the agent.")
    completed_files: List[str] = Field(
        default=[],
        description="A list of file paths that have been successfully generated/edited."
    )
    generated_code_cache: Dict[str, str] = Field(
        default={},
        description="A cache of the generated content for files that have been completed. Key is file_path, value is the code."
    )
    start_time: str = Field(description="The ISO 8601 timestamp of when the run was started.")
    last_update_time: str = Field(description="The ISO 8601 timestamp of the last update to this run state.")
