        ...
        try:
            ...
            if not unprocessed_files:
                ...
                if push_result_url is not None:  # This means push was successful
                    ...
                    if not self._create_pull_request(analysis.branch_name, pr_title, pr_body):
                        # If gh fails or is not present, and we have a URL from git push (push_result_url is not empty)
                        if push_result_url:
                            self._log_info("`gh` command failed or not found. Opening the PR creation link from git push output...", icon="🌐")
