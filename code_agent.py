...
"""
            ...
            prompt += """
Please provide your analysis. Use the `git_grep_search_tool` and `read_file_tool` if you need to find specific code snippets or understand file contents. If you are not confident in your plan, request more grep queries by populating `additional_grep_queries_needed`. If you are blocked by ambiguity in the task, use `user_request`.
"""
...
All files edited in this session: {json.dumps([f["file_path"] for f in edited_files])}
...
class CliManager:
    ...
    def _reconcile_and_validate_analysis(
        ...
        analysis.files_to_create = sorted(
            new_files_to_create, key=lambda f: f.file_path
        )
    ...
    def _execute_generation_loop(
        self, analysis: CodeAnalysis, all_repo_files: List[str], app_desc_content: str
    ) -> List[str]:
        """
        ...
        retries = 0
        completed_files_data: List[Dict[str, Any]] = []

        # Initial status update for the generation view
        self.status_queue.put(
            {
                "status": "generating",
        ...
        while files_to_process and retries <= MAX_REANALYSIS_RETRIES:
            ...
            while i < len(files_to_process):
                ...
                remaining_order = files_to_process[i + 1 :]
                ...
                completed_files_data.append(completed_file_info)
                context_data[file_path] = generated_code.code  # Update context for next file

                # Increment index after successful processing
                i += 1
                pending_files = files_to_process[i:]

                self.status_queue.put(
                    {
                        "status": "generating",
                        "generation_order": analysis.generation_order,
                        "completed_files": completed_files_data,
                ...
                if generated_code.add_to_generation_queue:
                    ...
                    # Filter out files that are already in the queue or processed
                    processed_paths = {f["file_path"] for f in completed_files_data}
                    files_to_add = [
                        f
                        for f in files_to_add
                        if f not in files_to_process and f not in processed_paths
                    ]
                    if files_to_add:
                        self._log_info(
            ...
            # If reanalysis was needed, the files from the current index `i` onwards are unprocessed.
            # If the loop completed, `i` will equal `len(files_to_process)`, and this will be empty.
    ...
    def _report_final_status(self, unprocessed_files: List[str]):
        ...
        if unprocessed_files:
            self._log_error(
                f"Failed to complete the task after {MAX_REANALYSIS_RETRIES} retries."
            )
    ...
    def run(self):
        ...
        server = None
        if not self.args.force:
            viewer_html_path = create_code_agent_html_viewer(
                self.args.port, all_repo_files
            )
                ...
                def do_GET(self):
                    if self.path == "/status":
                        try:
                            ...
                            update = self.cli_manager.status_queue.get(
                                block=True, timeout=28
                            )
                            self._send_response(
                                200, "application/json", json.dumps(update).encode("utf-8")
                            )
            ...
            server = ApprovalWebServer(
                ("", self.args.port),
                StatusAwareApprovalHandler,
                html_file_path=os.path.realpath(viewer_html_path),
            )
        ...
        try:
            ...
            while True:  # This loop handles user feedback on the plan
                ...
                if user_feedback:
                    ...
                    while analysis_retries < MAX_ANALYSIS_GREP_RETRIES:
                        ...
                        if current_analysis.additional_grep_queries_needed:
                            analysis_retries += 1
                            self._log_info(
                                "AI has requested more information via git grep. Running queries...",
                                icon="🤖",
                            )
                            new_results = [
                                self.agent_tools.git_grep_search(q)
                                for q in current_analysis.additional_grep_queries_needed
                            ]
                            grep_results = "\n\n".join(new_results)
                            analysis = None
                        else:
                            ...
                            break
                    if not analysis:
                        error_msg = f"Failed to get a confident analysis after {MAX_ANALYSIS_GREP_RETRIES} attempts."
                        if server:
                            self.status_queue.put({"status": "error", "error": error_msg})
                ...
                if analysis.user_request:
                    self._log_info(
                        f"AI is asking a question: {analysis.user_request}", icon="❓"
                    )
                    if not self.args.force and server:
                        self.status_queue.put(
                            {
                                "status": "user_input_required",
                                "user_request": analysis.user_request,
                            }
                        )
                        decision, data = server.wait_for_decision()
                        if decision == "user_input":
                            user_response = (
                                data.get("user_input") if isinstance(data, dict) else None
                            )
                            if user_response:
                                ...
                                if server:
                                    self.status_queue.put(
                                        {"status": "error", "error": "User did not provide input."}
                                    )
                                return
                        else:
                            error_msg = f"Expected user input, but got decision '{decision}'. Aborting."
                            self._log_error(error_msg)
                            if server:
                                self.status_queue.put(
                                    {
                                        "status": "error",
                                        "error": f"Operation cancelled during user input. Decision: {decision}",
                                    }
                                )
                            return
                    else:  # --force is on or server not running
                        self._log_error(
                            "AI requested user input in non-interactive mode. Aborting."
                        )
                        return

                # C. Reconcile analysis
                if not self._reconcile_and_validate_analysis(analysis, all_repo_files):
                    error_msg = "Failed to reconcile the analysis plan. Aborting."
                    if server:
                        self.status_queue.put(
                            {"status": "error", "error": "Failed to reconcile the analysis plan."}
                        )
                    self._log_error(error_msg)
                    return
                ...
                if not analysis.generation_order:
                    ...
                    if server:
                        self.status_queue.put(
                            {
                                "status": "done",
                                "message": "AI analysis resulted in no files to change.",
                            }
                        )
                    return
                ...
                if not self.args.force and server:
                    self._log_info(
                        "Analysis complete. Awaiting user confirmation in browser.",
                        icon="✅",
                    )

                    payload = analysis.model_dump()
                    # The JS expects a single string for the plan, not a list.
                    payload["plan"] = "\n".join(analysis.plan)
                    payload["all_files"] = all_repo_files
                    payload["task"] = self.args.task

                    self.status_queue.put({"status": "plan_review", **payload})

                    decision, data = server.wait_for_decision()

                    if decision == "approve":
                        self._log_info(
                            "Plan approved by user. Proceeding with code generation.",
                            icon="✅",
                        )
                        if data and isinstance(data, dict):
                            # Handle additional context files
                            additional_files: List[str] = data.get("context_files", [])
                            if additional_files:
                                self._log_info(
                                    f"User added {len(additional_files)} files to context: {additional_files}"
                                )
                                # Add to relevant_files, avoiding duplicates
                                analysis.relevant_files = sorted(
                                    list(set(analysis.relevant_files + additional_files))
                                )

                        if not self._create_and_checkout_branch(analysis.branch_name):
                            self.status_queue.put(
                                {"status": "error", "error": "Could not create git branch."}
                            )
                        ...
                        if data and isinstance(data, dict):
                            ...
                            additional_files = data.get("context_files", [])
                            if additional_files:
                                self._log_info(
                                    f"User added {len(additional_files)} files to context for re-analysis: {additional_files}"
                                )
                                analysis.relevant_files = sorted(
                                    list(set(analysis.relevant_files + additional_files))
                                )
                        ...
                        if not user_feedback:
                            self._log_warning(
                                "Feedback submitted without text. Ignoring and awaiting new decision."
                            )
                            server.reset_decision()
                            continue
            ...
            unprocessed_files = self._execute_generation_loop(
                analysis, all_repo_files, app_desc_content
            )
            ...
            if not unprocessed_files:
                ...
                push_result_url = self._commit_and_push_changes(
                    analysis.branch_name, commit_message
                )
                if push_result_url is not None:  # This means push was successful
                    ...
                    if not self._create_pull_request(
                        analysis.branch_name, pr_title, pr_body
                    ) and push_result_url:
                    ...
                    if server:
                        self.status_queue.put({
                            "status": "done",
                            "message": "Task completed successfully. A pull request has been created.",
                    ...
                    if server:
                        self.status_queue.put({
                            "status": "error",
                            "error": "Code generation was successful, but the git push operation failed."
                        })

            else:
                self._log_warning(
                    "Some files were not processed. Skipping git commit and PR creation."
                )
                if server:
                    self.status_queue.put({
                        "status": "error",
                        "error": f"The agent failed to process all files after multiple retries. Unprocessed files: {', '.join(unprocessed_files)}"
                    })
